...
class MonPresensi_model extends CI_Model {
	...
	function get_akses_menu($tipe){
		$data = $this->db->query("SELECT * FROM ABSEN_USERSUBMENU JOIN ABSEN_AKSESMENU ON ABSEN_AKSESMENU.MENU=ABSEN_USERSUBMENU.ID WHERE ABSEN_AKSESMENU.ROLE_ID='".$tipe."' ORDER BY ABSEN_USERSUBMENU.CONTROLLER ASC, ABSEN_USERSUBMENU.MENU_ID ASC")->result();
		return $data;
	}
